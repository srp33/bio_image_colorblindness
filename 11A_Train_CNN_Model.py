...
def build_model(image_size, include_class_weighting, early_stopping, random_rotation, dropout, transfer_learning_model, fine_tuning):
    ...
    if early_stopping:
        #keras.callbacks.ModelCheckpoint(os.path.join(out_folder, "checkpoint_{epoch}.h5")),
        callbacks = [
...
training_images_tsv_file_paths = sys.argv[1].split(",")
out_folder = sys.argv[2]

os.makedirs(out_folder, exist_ok=True)

out_model_file_path = os.path.join(out_folder, "model.h5")
out_epoch_metrics_file_path = os.path.join(out_folder, "epoch_metrics.tsv")

if os.path.exists(out_model_file_path):
    print(f"A file already exists at {out_model_file_path}. If you want to rebuild the model, you must remove that file.")
    sys.exit(0)
...
for training_images_tsv_file_path in training_images_tsv_file_paths:
    training_df = pd.read_csv(training_images_tsv_file_path, delimiter="\t")
